...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.zh_backgroundColorPicker = ThemePickerWithKey(@"color01");
    
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    button.frame = CGRectMake(0, 0, 50, 35);
    button.imageView.contentMode = UIViewContentModeScaleAspectFit;
    [button zh_setImagePicker:ThemePickerWithKey(@"image04") forState:UIControlStateNormal];
    ...
    _scrollView.zh_backgroundColorPicker = ThemePickerWithKey(@"color01");
    ...
    [colors enumerateObjectsUsingBlock:^(NSString *hexString, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        progressView.layer.zh_borderWidthPicker = ThemePickerWithDict(d);
    }];
}
